...
// ============================================================
// ============================================================
// Render de Avatares Estilo Habbo / Sims (Fase de Estilo)
// ============================================================
void drawEntity3D(const Entity& e, double time) {
    float wx = e.renderX * 2.0f + 1.0f;
    float wz = e.renderY * 2.0f + 1.0f;
    float wy = 0.8f; // altura del centro del personaje

    float bob = sinf(time * 6.0f + e.id * 2.0f) * 0.04f; // rebote al respirar/caminar
    Vector3 pos = {wx, wy + bob, wz};

    // Sombra en el suelo (cilindro plano simulando sombra)
    DrawCylinder({wx, 0.02f, wz}, 0.4f, 0.4f, 0.01f, 16, alpha(BLACK, 80));

    // Cuerpo / Torso (Caja de color según rol)
    Color shirtColor = e.color;
    if (e.hasCriticalError || e.status == Status::Error) shirtColor = RED;
    else if (e.status == Status::Busy) shirtColor = ORANGE;

    DrawCube({wx, wy - 0.2f + bob, wz}, 0.6f, 0.7f, 0.3f, shirtColor);
    DrawCubeWires({wx, wy - 0.2f + bob, wz}, 0.6f, 0.7f, 0.3f, BLACK);

    // Cabeza (Esfera o cubo color piel simulando avatar clásico)
    Vector3 headPos = {wx, wy + 0.35f + bob, wz};
    DrawCube(headPos, 0.45f, 0.45f, 0.45f, {255, 220, 177, 255}); // tono piel
    DrawCubeWires(headPos, 0.45f, 0.45f, 0.45f, BLACK);

    // Pelo / Gorra superior según tipo
    Vector3 hairPos = {wx, wy + 0.6f + bob, wz};
    Color hairColor = (e.type == EntityType::Human) ? GOLD : DARKGRAY;
    DrawCube(hairPos, 0.47f, 0.15f, 0.47f, hairColor);

    // Nombre flotante y rol sobre la cabeza
    Vector3 textPos = {wx, wy + 1.0f + bob, wz};
    Vector2 screenPos = GetWorldToScreen(textPos, g_camera3D);
    (void)screenPos;
}

// ============================================================
// Render 3D Estilo Habbo / Sims
// ============================================================
void drawScene3D(const std::vector<Entity>& entities, Camera3D& camera, double time) {
    ...
    // Suelo de oficina cálido tipo madera/cerámica clara (Habbo style)
    float mapSize = 16.0f * 2.0f;
    // Suelo principal beige/madera clara
    DrawPlane({mapSize / 2.0f - 1.0f, 0.0f, mapSize / 2.0f - 1.0f}, {mapSize, mapSize}, {240, 230, 210, 255});

    // Cuadrícula sutil de baldosas
    for (int x = 0; x < 16; x++) {
        for (int z = 0; z < 16; z++) {
            Color tileCol = ((x + z) % 2 == 0) ? Color{230, 220, 200, 255} : Color{245, 238, 225, 255};
            DrawCubeWires({x * 2.0f + 1.0f, 0.01f, z * 2.0f + 1.0f}, 2.0f, 0.02f, 2.0f, {210, 200, 180, 100});
            (void)tileCol;
        }
    }

    // Paredes traseras de la habitación (Estilo Habbo isometric room)
    // Pared izquierda
    DrawCube({-0.1f, 2.0f, mapSize / 2.0f}, 0.2f, 4.0f, mapSize, {180, 200, 220, 255});
    // Pared fondo
    DrawCube({mapSize / 2.0f, 2.0f, -0.1f}, mapSize, 4.0f, 0.2f, {160, 185, 210, 255});

    // Zonas de la oficina amigables y coloridas (alfombras de colores pasteles)
    DrawCube({7.0f, 0.03f, 7.0f}, 13.8f, 0.04f, 13.8f, {210, 235, 250, 255}); // Desarrollo (Celeste pastel)
    DrawCube({23.0f, 0.03f, 7.0f}, 13.8f, 0.04f, 13.8f, {210, 245, 225, 255}); // Servidores (Verde pastel)
    DrawCube({7.0f, 0.03f, 23.0f}, 13.8f, 0.04f, 13.8f, {235, 215, 245, 255}); // Conferencia (Lila pastel)
    DrawCube({23.0f, 0.03f, 23.0f}, 13.8f, 0.04f, 13.8f, {255, 235, 210, 255}); // Lounge (Mandarina pastel)

    // Mobiliario decorativo básico estilo cubos de colores (Escritorios y racks)
    // Escritorio Dev
    DrawCube({6.0f, 0.6f, 6.0f}, 3.0f, 1.2f, 1.5f, {139, 90, 43, 255}); // Madera marrón
    // Racks servidores
    DrawCube({23.0f, 1.2f, 6.0f}, 2.0f, 2.4f, 1.0f, {50, 60, 70, 255}); 
    // Mesa reuniones
    DrawCube({6.0f, 0.5f, 22.0f}, 4.0f, 1.0f, 2.5f, {160, 110, 60, 255});

    // Dibujar avatares estilo Sims / Habbo
    ...
    // Etiquetas y burbujas 2D proyectadas
    for (const auto& e : entities) {
        float wx = e.renderX * 2.0f + 1.0f;
        float wz = e.renderY * 2.0f + 1.0f;
        Vector3 textPos = {wx, 2.3f, wz};
        ...
        if (screenPos.x > 0 && screenPos.y > 0 && screenPos.x < screenW && screenPos.y < screenH) {
            ...
            DrawRectangle((int)screenPos.x - tw/2 - 8, (int)screenPos.y - 14, tw + 16, 22, alpha(WHITE, 230));
            DrawRectangleLines((int)screenPos.x - tw/2 - 8, (int)screenPos.y - 14, tw + 16, 22, DARKGRAY);
            DrawText(roleName, (int)screenPos.x - tw/2, (int)screenPos.y - 11, 12, DARKBLUE);
            ...
            if (e.speech.expiry > time && !e.speech.text.empty()) {
                ...
                DrawRectangle((int)screenPos.x - bw/2 - 10, (int)screenPos.y - 42, bw + 20, 24, alpha(LIGHTGRAY, 240));
                DrawRectangleLines((int)screenPos.x - bw/2 - 10, (int)screenPos.y - 42, bw + 20, 24, GRAY);
                DrawText(bubble.c_str(), (int)screenPos.x - bw/2, (int)screenPos.y - 39, 10, BLACK);
            }
        }
    }
}
